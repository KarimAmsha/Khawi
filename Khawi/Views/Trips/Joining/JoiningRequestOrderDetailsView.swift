//
//  JoiningRequestOrderDetailsView.swift
//  Khawi
//
...
struct JoiningRequestOrderDetailsView: View {
                            ...
                            HStack(spacing: 16) {
                                VStack(alignment: .leading) {
                                    Text(LocalizedStringKey.carType)
                                        .customFont(weight: .book, size: 11)
                                        .foregroundColor(.grayA4ACAD())
                                    Text(viewModel.order?.user?.carType ?? "")
                                        .customFont(weight: .book, size: 14)
                                        .foregroundColor(.black141F1F())
                                }
                                .frame(maxWidth: .infinity, alignment: .leading)
                                ...
                                VStack(alignment: .leading) {
                                    Text(LocalizedStringKey.carModel)
                                        .customFont(weight: .book, size: 11)
                                        .foregroundColor(.grayA4ACAD())
                                    Text(viewModel.order?.user?.carModel ?? "")
                                        .customFont(weight: .book, size: 14)
                                        .foregroundColor(.black141F1F())
                                }
                                .frame(maxWidth: .infinity, alignment: .leading)
                            ...
                            HStack(spacing: 16) {
                                VStack(alignment: .leading) {
                                    Text(LocalizedStringKey.carColor)
                                        .customFont(weight: .book, size: 11)
                                        .foregroundColor(.grayA4ACAD())
                                    Text(viewModel.order?.user?.carColor ?? "")
                                        .customFont(weight: .book, size: 14)
                                        .foregroundColor(.black141F1F())
                                }
                                .frame(maxWidth: .infinity, alignment: .leading)
                                ...
                                VStack(alignment: .leading) {
                                    Text(LocalizedStringKey.carNumber)
                                        .customFont(weight: .book, size: 11)
                                        .foregroundColor(.grayA4ACAD())
                                    Text(viewModel.order?.user?.carNumber ?? "")
                                        .customFont(weight: .book, size: 14)
                                        .foregroundColor(.black141F1F())
                                }
                                .frame(maxWidth: .infinity, alignment: .leading)
                        ...
                        if let order = viewModel.order, let user = order.user, let offers = order.offers {
                            ...
                            if settings.id == user._id {
                                ...
                                if order.orderStatus == .new || order.orderStatus == .accepted {
                                    // Handle canceled status by driver
                                    Button {
                                        presentCancellationReasonPopup(status: .canceledByDriver)
                                    } label: {
...
#Preview {
    JoiningRequestOrderDetailsView(settings: UserSettings(), orderID: "", router: MainRouter(isPresented: .constant(.main)))
}
